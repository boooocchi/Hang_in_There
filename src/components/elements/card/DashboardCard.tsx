...
const DashboardCard: React.FC<Props> = ({ children, title, link }) => {
  return (
    <Link
      href={link}
      className="flex gap-sm items-center  xs:h-full xs:w-[25%] rounded-md bg-gray  relative overflow-hidden shadow-[5px_10px_10px_-5px_rgba(0,0,0,0.3)] p-md max-xs:px-sm hover:bg-middleGreen hover:text-gray transition-color duration-300 w-full group"
    >
